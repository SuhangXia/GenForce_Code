...
from sccwm_force_grounded_v22.eval.eval_sccwm_force_grounded_direct_v22 import (
    STRICT_CROSS_BAND_PROTOCOLS,
    STANDARD_PROTOCOLS,
    build_force_grounded_v21a_eval_loader,
    load_force_grounded_v22_for_eval,
)
from sccwm_force_grounded_v22.models import EMBEDDING_VIEW_TO_KEY, select_embedding_view_v22
...
def _collect_pair_rows(
    ...
    *,
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"Collect v22 probe rows {split} {protocol or 'all'} {embedding_view}", total=len(loader), leave=False)
        for batch in iterator:
            ...
            source_embedding = select_embedding_view_v22(source, embedding_view)
            target_embedding = select_embedding_view_v22(target, embedding_view)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train frozen post-hoc leakage probes on conservative force-grounded SCCWM v22 embeddings.")
    parser.add_argument("--config", type=str, default="sccwm_force_grounded_v22/configs/sccwm_stage2_force_grounded_v22ts.yaml")
