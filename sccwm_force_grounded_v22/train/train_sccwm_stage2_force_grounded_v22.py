...
from sccwm_force_grounded_v22.losses import compute_force_grounded_v22_losses, summarize_force_grounded_v22_batch
from sccwm_force_grounded_v22.models import build_force_grounded_v22_model
...
def _stage_name(cfg: dict[str, Any]) -> str:
    return str(cfg.get("experiment", {}).get("name", "stage2_force_grounded_v22"))


def build_force_grounded_v22_model_for_train(cfg: dict[str, Any], device: torch.device) -> torch.nn.Module:
    model = build_force_grounded_v22_model(cfg.get("model", {}))
    return model.to(device)


def _preflight_forward_metrics_v22(
    ...
    with torch.no_grad():
        ...
        loss_out = compute_force_grounded_v22_losses(outputs=outputs, batch=batch, loss_cfg=loss_cfg)
    return {key: float(value.item()) for key, value in loss_out.metrics.items()}


def _print_preflight_summary_v22(stage_name: str, label: str, summary: dict[str, Any], forward_metrics: dict[str, float]) -> None:
    ...
    if "trajectory_window_target_stats" in summary:
        print(f"[{stage_name}] preflight {label}: trajectory_window_target_stats={summary['trajectory_window_target_stats']}")
    ...
    print(
        ...
        f"force_penetration_proxy={forward_metrics.get('force_penetration_proxy', float('nan')):.4f} "
        f"force_contact_intensity={forward_metrics.get('force_contact_intensity', float('nan')):.4f} "
        f"force_load_progress={forward_metrics.get('force_load_progress', float('nan')):.4f} "
        f"force_penetration_ranking={forward_metrics.get('force_penetration_ranking', float('nan')):.4f} "
        f"force_latent_consistency_global={forward_metrics.get('force_latent_consistency_global', float('nan')):.4f} "
        f"force_latent_consistency_map={forward_metrics.get('force_latent_consistency_map', float('nan')):.4f} "
        ...
        f"trajectory_window_shape={forward_metrics.get('trajectory_window_shape', float('nan')):.4f} "
        f"trajectory_window_order={forward_metrics.get('trajectory_window_order', float('nan')):.4f} "
        f"trajectory_dwell_flat={forward_metrics.get('trajectory_dwell_flat', float('nan')):.4f} "
        f"trajectory_anchor_ranking={forward_metrics.get('trajectory_anchor_ranking', float('nan')):.4f} "
        f"contact_support_entropy={forward_metrics.get('contact_support_entropy', float('nan')):.4f} "
        f"contact_support_confidence={forward_metrics.get('contact_support_confidence', float('nan')):.4f} "
        f"contact_support_spread={forward_metrics.get('contact_support_spread', float('nan')):.4f}"
    )


def run_force_grounded_v22_epoch(
    stage_name: str,
    model: torch.nn.Module,
...
def train_force_grounded_stage2_v22(cfg: dict[str, Any], *, sanity_only: bool = False) -> None:
    ...
    model = build_force_grounded_v22_model_for_train(cfg, device)
    ...
    train_preflight_summary = summarize_force_grounded_v22_batch(train_preflight_batch, loss_cfg)
    val_preflight_summary = summarize_force_grounded_v22_batch(val_preflight_batch, loss_cfg)
    train_preflight_forward = _preflight_forward_metrics_v22(model, train_preflight_batch, loss_cfg, device)
    val_preflight_forward = _preflight_forward_metrics_v22(model, val_preflight_batch, loss_cfg, device)
    preflight_report = {
        "stage": stage_name,
    ...
    _print_preflight_summary_v22(stage_name, "train", train_preflight_summary, train_preflight_forward)
    _print_preflight_summary_v22(stage_name, "val", val_preflight_summary, val_preflight_forward)
    if sanity_only:
        ...
        for epoch in range(start_epoch, total_epochs + 1):
            ...
            if epoch == preflight_epoch:
                ...
                if epoch_episode_subset is not None:
                    print(
                        f"[{stage_name}] epoch={epoch} sampled {len(epoch_episode_subset)} train episodes for this epoch "
                        f"(dynamic episode subset enabled)"
                    )
            ...
            train_metrics = run_force_grounded_v22_epoch(
                stage_name,
                model,
                train_loader,
                optimizer,
            ...
            with torch.no_grad():
                val_metrics = run_force_grounded_v22_epoch(
                    stage_name,
                    model,
                    val_loader,
                    None,
...
def main() -> None:
    parser = build_stage_argparser(
        "Train conservative force-grounded SCCWM v22 ablations.",
    ...
    train_force_grounded_stage2_v22(cfg, sanity_only=bool(args.sanity_only))
