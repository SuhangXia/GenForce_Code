...
def square_to_padded_canvas(image: Image.Image, *, mode: str, resample: int) -> Image.Image:
    if mode not in {"RGB", "L"}:
        raise ValueError(f"Unsupported mode: {mode}")
    canvas = Image.new(mode, (TARGET_WIDTH, TARGET_HEIGHT), 0)
    if mode == "RGB":
        canvas = Image.new(mode, (TARGET_WIDTH, TARGET_HEIGHT), (0, 0, 0))
    resized = image.resize((TARGET_SQUARE, TARGET_SQUARE), resample=resample)
    offset_x = (TARGET_WIDTH - TARGET_SQUARE) // 2
...
def prepare_marker_image(source_path: Path) -> Image.Image:
    ...
    return square_to_padded_canvas(marker, mode="L", resample=PIL_RESAMPLING.NEAREST)
...
def main() -> None:
    ...
    try:
        ...
        logical_pairs, manifest = build_logical_pairs(stage_root, args.scale_l_mm, args.scale_r_mm)
