...
from sccwm_force_grounded_v2.losses import compute_force_grounded_losses
...
def build_force_grounded_model(cfg: dict[str, Any], device: torch.device) -> SCCWMForceGrounded:
...
def _select_overlay_root(ds_cfg: dict[str, Any], *, split: str, kind: str) -> Path:
    ...
    if root_value is None:
        fallback_key = "dataset_a_root" if kind == "image" else "dataset_a_assets_root"
        root_value = ds_cfg.get(fallback_key)
    if root_value is None:
...
def run_force_grounded_epoch(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(iterator):
        ...
        loss_out = compute_force_grounded_losses(outputs=outputs, batch=batch, loss_cfg=loss_cfg)
...
def train_force_grounded_stage2(cfg: dict[str, Any]) -> None:
    ...
    model = build_force_grounded_model(cfg, device)
    ...
    wandb.init(
        ...
        name=str(cfg.get("system", {}).get("wandb_name", cfg.get("experiment", {}).get("name", "sccwm_force_grounded_v2"))),
    ...
    try:
        for epoch in range(start_epoch, total_epochs + 1):
            ...
            if epoch_episode_subset is not None:
                print(
                    f"[stage2_force_grounded] epoch={epoch} sampled {len(epoch_episode_subset)} train episodes for this epoch "
            ...
            train_metrics = run_force_grounded_epoch(
                model,
                train_loader,
                optimizer,
                device,
                loss_cfg=loss_cfg,
                desc=f"Train stage2_force_grounded {epoch}",
                progress=not bool(train_cfg.get("no_progress", False)),
                max_batches=max_train_batches,
            )
            with torch.no_grad():
                val_metrics = run_force_grounded_epoch(
                    model,
                    val_loader,
                    None,
                    device,
                    loss_cfg=loss_cfg,
                    desc=f"Val stage2_force_grounded {epoch}",
                    progress=not bool(train_cfg.get("no_progress", False)),
            ...
            print(f"[stage2_force_grounded] saved last checkpoint -> {output_dir / 'last.pt'}")
            if improved:
                best_value = current
                save_checkpoint(output_dir / "best.pt", payload)
                print(
                    f"[stage2_force_grounded] updated best checkpoint -> {output_dir / 'best.pt'} "
            ...
            if int(train_cfg.get("save_every", 0)) > 0 and epoch % int(train_cfg.get("save_every", 1)) == 0:
                ...
                print(f"[stage2_force_grounded] saved periodic checkpoint -> {output_dir / f'epoch_{epoch:04d}.pt'}")
            ...
            print(
                f"[stage2_force_grounded] epoch={epoch} train_loss={train_metrics.get('loss', 0.0):.4f} "
...
def main() -> None:
    parser = build_stage_argparser(
        "Train the force-grounded SCCWM v2 stage-2 model.",
    ...
    train_force_grounded_stage2(cfg)
