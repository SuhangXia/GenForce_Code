...
from sccwm_force_grounded_v2.datasets import OverlayPairedSequenceDataset
from sccwm_force_grounded_v2.models import SCCWMForceGrounded
...
def load_force_grounded_for_eval(cfg: dict[str, Any], checkpoint_path: str | Path, device: torch.device) -> SCCWMForceGrounded:
    model_cfg = cfg.get("model", {})
    model = SCCWMForceGrounded(
...
def build_force_grounded_eval_loader(cfg: dict[str, Any], *, split: str, sequence_length: int | None, device: torch.device) -> DataLoader[Any]:
...
def run_force_grounded_direct_eval(
    cfg: dict[str, Any],
    *,
    ...
    if protocol in STRICT_CROSS_BAND_PROTOCOLS:
        return _run_force_grounded_cross_band_eval(
            cfg,
            model=model,
            protocol=protocol,
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"ForceGrounded direct eval {protocol} {split}", total=len(loader), leave=False)
        _progress_write(
            iterator,
            f"force_grounded_direct_eval started protocol={protocol} split={split} checkpoint={checkpoint_path} "
        ...
        for batch_idx, batch in enumerate(iterator, start=1):
            ...
            if now >= next_log_time:
                _progress_write(
                    iterator,
                    f"force_grounded_direct_eval heartbeat protocol={protocol} split={split} "
    ...
    _progress_write(
        iterator,
        f"force_grounded_direct_eval finished protocol={protocol} split={split} records={len(records)} "
        f"filtered={result['filtered_sample_count']} elapsed={_format_elapsed(__import__('time').time() - started_at)}",
    )
    return result


def _run_force_grounded_cross_band_eval(
    cfg: dict[str, Any],
    *,
    model: SCCWMForceGrounded,
    protocol: str,
    ...
    started_at = __import__("time").time()
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"ForceGrounded cross-band eval {protocol}", total=len(loader), leave=False)
        _progress_write(
            iterator,
            f"force_grounded_cross_band_eval started protocol={protocol} matched_pairs={len(pair_specs)} "
            f"total_batches={len(loader)} limit={limit if limit > 0 else 'none'}",
        )
        next_log_time = started_at + log_interval_seconds
        for batch_idx, batch in enumerate(iterator, start=1):
            ...
            if now >= next_log_time:
                _progress_write(
                    iterator,
                    f"force_grounded_cross_band_eval heartbeat protocol={protocol} "
                    f"batch={batch_idx}/{len(loader)} records={len(records)} elapsed={_format_elapsed(now - started_at)}",
                )
                while next_log_time <= now:
                    next_log_time += log_interval_seconds
    ...
    result["model_type"] = "sccwm_force_grounded_v2"
    _progress_write(
        iterator,
        f"force_grounded_cross_band_eval finished protocol={protocol} records={len(records)} "
...
def main() -> None:
    parser = build_eval_argparser(
        "Evaluate force-grounded SCCWM v2 direct prediction protocols.",
    ...
    result = run_force_grounded_direct_eval(
        cfg,
    ...
    output = args.output or f"sccwm/eval_outputs/sccwm_force_grounded_v2_{args.protocol}_{args.split}.json"
