...
class RealTimeVisual():
    def __init__(self, num_channels, x_timeMax, frames, interval, baseline_count,task,imsv,save_file_name,client_socket):
        ...
        self.instance.createCalibration(CalFilePath='Nano17/FT31439.cal', index=1)
    ...
    def data_save(self,ft_readout,num_saved_images,stamp):
    ...
    def update(self,frame):
        ...
        if(len(stamp)>0):
            ...
            for i in range(self.num_channels):
                ...
                num_saved_images = self.imsv.save_images(stamp)
                self.data_save(ft_readout,num_saved_images,stamp)

        # end_time = time.time()  # Record the end time
        # time_cost = end_time - start_time
        # print(f"Time cost for frame {frame}: {time_cost} seconds")
