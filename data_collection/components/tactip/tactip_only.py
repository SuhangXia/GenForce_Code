...
class ImageSee():
    ...
    def options(self):
        defult_save_dir = f"components/tactip/data/{self.current_date}"
        parser = argparse.ArgumentParser()
        parser.add_argument("--config",type=str,default="config/tactip.yaml")
    ...
    def save_images(self):
        ...
        while True:
            ...
            cv2.imshow(curr_window_name, img_crop)
            # resize_func = self.resizeF()
            # img_crop = resize_func(frame)
            key = cv2.waitKey(1)
            if key == 115:
...
if __name__ == "__main__":
    ...
    print(f"Current exposure value: {current_exposure}")  
    print(f"current_brightness: {current_brightness}")  
    print(f"current_contrast: {current_contrast}")  
    print(f"current_sharpness: {current_sharpness}")  
    ImSv = ImageSee(video)
    ImSv.ref_capture()
    ImSv.save_images()
