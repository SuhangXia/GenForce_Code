from ur5e_tactip import *
import time

if __name__ == '__main__':
    
    collect_calib = False
    collect_modulus = True
    sensor = "hetero/location_pair/18types/tactip/modulus"  
    # sensor = "hetero/location_pair/18types/tactip/data"  
    with open(f"components/tactip/config/hetero_tactip.yaml",'r') as fp:  
        config = yaml.safe_load(fp)  
    # depth = config["modulus"]["depth"]
    ctime = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")  
    indenter = config["Indenter"]  
    # dir_ft = f"data/{sensor}/{depth}/force/{indenter}-{ctime}"  
    # dir_imgs = f"data/{sensor}/{depth}/image/{indenter}-{ctime}"  
    # dir_pos = f"data/{sensor}/{depth}/pose/{indenter}-{ctime}"  
    # dir_path = f'data/{sensor}/{depth}/path/{indenter}-{ctime}' 
