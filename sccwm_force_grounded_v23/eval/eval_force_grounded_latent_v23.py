...
from sccwm_force_grounded_v23.eval.eval_sccwm_force_grounded_direct_v23 import load_force_grounded_v23_for_eval
...
def _summarize_filtered_rows_v23(rows: list[dict[str, Any]], *, protocol: str) -> dict[str, Any]:
    if not rows:
    ...
    metrics = {
        ...
        "penetration_proxy_mae": float(sum(abs(row["pred_penetration_proxy"] - row["penetration_target"]) for row in rows) / len(rows)),
        "penetration_proxy_pearson": _pearson([row["pred_penetration_proxy"] for row in rows], [row["penetration_target"] for row in rows]),
        "contact_intensity_proxy_mae": float(sum(abs(row["pred_contact_intensity_proxy"] - row["contact_target"]) for row in rows) / len(rows)),
        "contact_intensity_proxy_pearson": _pearson([row["pred_contact_intensity_proxy"] for row in rows], [row["contact_target"] for row in rows]),
        "load_progress_proxy_mae": float(sum(abs(row["pred_load_progress_proxy"] - row["load_progress_target"]) for row in rows) / len(rows)),
        "load_progress_proxy_pearson": _pearson([row["pred_load_progress_proxy"] for row in rows], [row["load_progress_target"] for row in rows]),
        "scale_adv_accuracy": float(sum(row.get("scale_adv_correct", 0.0) for row in rows) / len(rows)),
        "branch_adv_accuracy": float(sum(row.get("branch_adv_correct", 0.0) for row in rows) / len(rows)),
        "sample_count": len(rows),
    }
    if "geometry_raw_source" in rows[0]:
        metrics["geometry_raw_pair_cosine"] = _cosine_mean([row["geometry_raw_source"] for row in rows], [row["geometry_raw_target"] for row in rows])
    if "geometry_deconf_source" in rows[0]:
        metrics["geometry_deconf_pair_cosine"] = _cosine_mean(
            [row["geometry_deconf_source"] for row in rows],
            [row["geometry_deconf_target"] for row in rows],
        )
    if "geometry_canonical_source" in rows[0]:
        metrics["geometry_canonical_pair_cosine"] = _cosine_mean(
            [row["geometry_canonical_source"] for row in rows],
            [row["geometry_canonical_target"] for row in rows],
        )
    if "operator_source" in rows[0]:
        metrics["operator_pair_cosine"] = _cosine_mean([row["operator_source"] for row in rows], [row["operator_target"] for row in rows])
        metrics["operator_scale_accuracy"] = float(sum(row.get("operator_scale_correct", 0.0) for row in rows) / len(rows))
        metrics["operator_branch_accuracy"] = float(sum(row.get("operator_branch_correct", 0.0) for row in rows) / len(rows))
    return {
...
def run_force_grounded_latent_eval_v23(
    cfg: dict[str, Any],
    *,
    ...
    model = load_force_grounded_v23_for_eval(cfg, checkpoint_path, device)
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"ForceGroundedV23 latent eval {protocol}", total=len(loader), leave=False)
        for batch in iterator:
            ...
            operator_scale_correct = None
            operator_branch_correct = None
            if "operator_scale_logits" in source and "operator_scale_logits" in target:
            ...
            for sample_idx in range(batch_n):
                ...
                if "geometry_latent_raw" in source:
                    row["geometry_raw_source"] = source["geometry_latent_raw"][sample_idx].detach().cpu().tolist()
                    row["geometry_raw_target"] = target["geometry_latent_raw"][sample_idx].detach().cpu().tolist()
                if "geometry_latent_deconf" in source:
                    row["geometry_deconf_source"] = source["geometry_latent_deconf"][sample_idx].detach().cpu().tolist()
                    row["geometry_deconf_target"] = target["geometry_latent_deconf"][sample_idx].detach().cpu().tolist()
                if "geometry_latent_canonical" in source:
                    row["geometry_canonical_source"] = source["geometry_latent_canonical"][sample_idx].detach().cpu().tolist()
                    row["geometry_canonical_target"] = target["geometry_latent_canonical"][sample_idx].detach().cpu().tolist()
                if "operator_code" in source:
                    row["operator_source"] = source["operator_code"][sample_idx].detach().cpu().tolist()
                    row["operator_target"] = target["operator_code"][sample_idx].detach().cpu().tolist()
                    if operator_scale_correct is not None:
                        row["operator_scale_correct"] = float(0.5 * (operator_scale_correct[sample_idx].item() + operator_scale_correct[sample_idx + batch_n].item()))
                    if operator_branch_correct is not None:
                        row["operator_branch_correct"] = float(0.5 * (operator_branch_correct[sample_idx].item() + operator_branch_correct[sample_idx + batch_n].item()))
                rows.append(row)
    ...
    result = _summarize_filtered_rows_v23(filtered, protocol=protocol)
    result["rows"] = filtered
    result["model_type"] = str(cfg.get("model", {}).get("variant", "v23"))
...
def main() -> None:
    parser = build_eval_argparser(
        "Evaluate geometry/operator latent behavior for force-grounded SCCWM v23 variants.",
    ...
    result = run_force_grounded_latent_eval_v23(
        cfg,
    ...
    output = args.output or f"sccwm/eval_outputs/sccwm_force_grounded_v23_latent_{args.protocol}_{args.split}.json"
