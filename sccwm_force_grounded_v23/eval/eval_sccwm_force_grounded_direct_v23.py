...
from sccwm_force_grounded_v23.models import EMBEDDING_VIEW_TO_KEY, build_force_grounded_v23_model, select_embedding_view_v23
...
def load_force_grounded_v23_for_eval(cfg: dict[str, Any], checkpoint_path: str | Path, device: torch.device) -> torch.nn.Module:
    model = build_force_grounded_v23_model(cfg.get("model", {})).to(device)
...
def _run_force_grounded_eval_loop_v23(
    *,
    cfg: dict[str, Any],
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"ForceGroundedV23 direct eval {protocol} {split_label}", total=len(loader), leave=False)
        prefix = "force_grounded_v23_cross_band_eval" if strict_cross_band else "force_grounded_v23_direct_eval"
        _progress_write(
            iterator,
        ...
        for batch_idx, batch in enumerate(iterator, start=1):
            ...
            source_embedding = select_embedding_view_v23(source, embedding_view)
            target_embedding = select_embedding_view_v23(target, embedding_view)
...
def run_force_grounded_direct_eval_v23(
    cfg: dict[str, Any],
    *,
    checkpoint_path: str | Path,
    split: str,
    ...
    if protocol in STRICT_CROSS_BAND_PROTOCOLS:
        ...
        result = _run_force_grounded_eval_loop_v23(
            cfg=cfg,
            model=model,
            loader=loader,
            protocol=protocol,
...
def main() -> None:
    parser = build_eval_argparser(
        "Evaluate geometry-focused force-grounded SCCWM v23 direct prediction protocols.",
    ...
    result = run_force_grounded_direct_eval_v23(
        cfg,
    ...
    output = args.output or f"sccwm/eval_outputs/sccwm_force_grounded_v23_{args.protocol}_{args.split}_{args.embedding_view}.json"
