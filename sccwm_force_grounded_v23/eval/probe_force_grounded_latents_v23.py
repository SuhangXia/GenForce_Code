...
from sccwm_force_grounded_v23.models import EMBEDDING_VIEW_TO_KEY, select_embedding_view_v23
...
def _collect_pair_rows(
    ...
    *,
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"Collect v23 probe rows {split} {protocol or 'all'} {embedding_view}", total=len(loader), leave=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train frozen post-hoc leakage probes on geometry-focused force-grounded SCCWM v23 embeddings.")
    parser.add_argument("--config", type=str, default="sccwm_force_grounded_v23/configs/sccwm_stage2_force_grounded_v23gof.yaml")
    ...
    parser.add_argument("--embedding-view", type=str, default="latent_only", choices=list(EMBEDDING_VIEW_TO_KEY.keys()))
