...
from sccwm_force_grounded_v24.eval.eval_sccwm_force_grounded_direct_v24 import load_force_grounded_v24_for_eval
...
def _summarize_filtered_rows_v24(rows: list[dict[str, Any]], *, protocol: str) -> dict[str, Any]:
    if not rows:
    ...
    if "canonical_force_projected_source" in rows[0]:
        ...
        metrics["canonical_force_alignment_cosine"] = 0.5 * (
...
def run_force_grounded_latent_eval_v24(
    cfg: dict[str, Any],
    *,
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"ForceGroundedV24 latent eval {protocol}", total=len(loader), leave=False)
            ...
            for sample_idx in range(batch_n):
                row = {
                    **_latent_row_common(batch, sample_idx),
                    ...
                    "geometry_raw_source": source["geometry_latent_raw"][sample_idx].detach().cpu().tolist(),
                    "geometry_raw_target": target["geometry_latent_raw"][sample_idx].detach().cpu().tolist(),
                    "geometry_canonical_source": source["geometry_latent_canonical"][sample_idx].detach().cpu().tolist(),
                    "geometry_canonical_target": target["geometry_latent_canonical"][sample_idx].detach().cpu().tolist(),
    ...
    result = _summarize_filtered_rows_v24(filtered, protocol=protocol)
...
def main() -> None:
    parser = build_eval_argparser(
        "Evaluate geometry/operator/force latent behavior for force-guided SCCWM v24.",
    ...
    result = run_force_grounded_latent_eval_v24(
        cfg,
    ...
    output = args.output or f"sccwm/eval_outputs/sccwm_force_grounded_v24_latent_{args.protocol}_{args.split}.json"
