...
class Visualizer:
    def __init__(self, ros_pub, ros_points_pub, bridge):
        ...
        self.ros_points_pub = ros_points_pub
        self.bridge = bridge
    ...
    def update(self, frame):
        try:
            if not data_queue.empty():
                ...
                delta_x = np.clip(points[:, 0]/5000, -0.6, 0.6)  
                delta_y = np.clip(points[:, 1]/5000, -0.6, 0.6)  
                plot_x = GRID_X + delta_x  
                plot_y = GRID_Y + delta_y  
                sizes = 300 + points[:, 2]/2.5
                sizes = np.clip(sizes, 300, 6000)  
                ...
                # --- NEW: ROS POINTS PUBLISHING ---
                # Publish points as PointCloud
                cloud_msg = PointCloud()
                cloud_msg.header = Header()
                cloud_msg.header.stamp = rospy.Time.now()
                cloud_msg.header.frame_id = "uskin_sensor"
                
                cloud_msg.points = [Point32(x=float(points[i, 0]), 
                                          y=float(points[i, 1]), 
                                          z=float(points[i, 2])) 
                                  for i in range(len(points))]
                
                self.ros_points_pub.publish(cloud_msg)
...
def main():
    ...
    ros_pub = rospy.Publisher('/uskin2.5/marker', Image, queue_size=1)
    ros_points_pub = rospy.Publisher('/uskin2.5/raw', PointCloud, queue_size=10)  # NEW: Points publisher
    bridge = CvBridge()
    ...
    vis = Visualizer(ros_pub, ros_points_pub, bridge)  # Pass both publishers
