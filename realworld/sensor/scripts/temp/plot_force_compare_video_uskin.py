...
class ForceVisualizer(QMainWindow):
    def __init__(self, ros_sensor_type=None, video_width=1200, video_height=800, video_name="default"):
        super().__init__()
        ...
        self.csv_writers = {}
        self.csv_files = {}
        self.csv_data_queues = {}

        # 录制时钟与节拍（使用墙钟，保持与实际时间一致）
        self.recording_start_time = None  # 使用 time.time()
        ...
        # 视频异步写入
        self.video_queue = deque(maxlen=300)
    ...
    def init_ui(self):
        ...
        self.sensor_combo.addItems(["AII", "uskin", "uskin2.5"])
        ...
        self.video_name_input = QLineEdit(self.video_name)
        ...
        self.fps_spinbox.setValue(60.0)
    ...
    def start_recording(self):
        ...
        try:
            ...
            # Create CSV folder and files
            csv_dir = os.path.join(video_dir, "csv")
            os.makedirs(csv_dir, exist_ok=True)

            # Initialize CSV data queues
            self.csv_data_queues = {
                'uskin_force': deque(),
                'uskin_raw': deque(), 
                'nano17_force': deque()
            ...
            # Create CSV files
            csv_files = {
                'uskin_force': os.path.join(csv_dir, f"{video_name}_{timestamp_str}_uskin_force.csv"),
                'uskin_raw': os.path.join(csv_dir, f"{video_name}_{timestamp_str}_uskin_raw.csv"),
                'nano17_force': os.path.join(csv_dir, f"{video_name}_{timestamp_str}_nano17_force.csv")
            }
            ...
            try:
                ...
                for key, filename in csv_files.items():
                    ...
                    if key == 'uskin_force':
                        writer.writerow(['timestamp', 'force_x', 'force_y', 'force_z'])
                    elif key == 'uskin_raw':
                        writer.writerow(['timestamp'] + [f'sensor_{i}_x' for i in range(16)] + 
                                    [f'sensor_{i}_y' for i in range(16)] + 
                                    [f'sensor_{i}_z' for i in range(16)])
                    elif key == 'nano17_force':
                        writer.writerow(['timestamp', 'force_x', 'force_y', 'force_z'])
                        
                    self.csv_files[key] = file_obj
                    self.csv_writers[key] = writer
                    
                print(f"[INFO] CSV files created in: {csv_dir}")
    ...
    def stop_recording(self):
        ...
        # Write remaining CSV data and close files
        try:
            for key in self.csv_data_queues:
                while self.csv_data_queues[key]:
                    row_data = self.csv_data_queues[key].popleft()
                    if key in self.csv_writers:
                        self.csv_writers[key].writerow(row_data)
                        
            for key in self.csv_files:
                if self.csv_files[key]:
                    self.csv_files[key].close()
                    
            print(f"[INFO] CSV data saved and files closed")
            
        except Exception as e:
            print(f"[ERROR] Error closing CSV files: {e}")

        # Clear CSV resources
        self.csv_writers.clear()
        self.csv_files.clear() 
        self.csv_data_queues.clear()
    ...
    def capture_frame_for_video(self):
        ...
        wrote_any = False
        while now >= (self.next_tick_time or now + 1e9):
            # 若没有 next_tick_time（安全兜底），初始化一次
            if self.next_tick_time is None:
            ...
            if len(self.video_queue) >= self.video_queue.maxlen:
                try:
                    self.video_queue.popleft()  # 丢弃最旧，保持实时
                except IndexError:
                    pass
    ...
    def setup_force_charts(self, layout):
        ...
        series_ros_fx.setName("uskin Fx")
        ...
        for i, chart in enumerate([chart_fx, chart_fy, chart_fz]):
            ...
            if i == 0:  # Fx
                ...
                axis_y.setRange(-4.0, 1.0)
                axis_y.setTickCount(6)          # -4, -3, -2, -1, 0,1
    ...
    def subscribe_to_ros_topics(self):
        ...
        try:
            raw_topic = roslibpy.Topic(self.ros_client, f'/{sensor_name}/raw', 'sensor_msgs/PointCloud')
            raw_topic.subscribe(self.on_uskin_raw_data)
            print(f"Subscribed to: /{sensor_name}/raw")
        except Exception as e:
            print(f"[ERROR] Failed to subscribe to raw topic: {e}")
    
    def on_uskin_raw_data(self, msg):
        """Handle uskin raw point cloud data"""
        if not self.save_video:
            return
        ...
        try:
            ...
            if len(points) >= 16:
                ...
                # Add all x coordinates (16 sensors)
                for i in range(16):
                    if i < len(points):
                        row_data.append(points[i].get('x', 0.0))
                    else:
                        row_data.append(0.0)
                
                # Add all y coordinates (16 sensors) 
                for i in range(16):
                    if i < len(points):
                        row_data.append(points[i].get('y', 0.0))
                    else:
                        row_data.append(0.0)
                        
                # Add all z coordinates (16 sensors)
                for i in range(16):
                    if i < len(points):
                        row_data.append(points[i].get('z', 0.0))
                    else:
                        row_data.append(0.0)
                
                # Add to queue
                if len(self.csv_data_queues.get('uskin_raw', [])) < 10000:
                    self.csv_data_queues['uskin_raw'].append(row_data)
                    
            else:
                print(f"[WARNING] Received {len(points)} points, expected 16")
                    
        except Exception as e:
            print(f"[ERROR] Processing uskin raw data: {e}")
            print(f"[DEBUG] Message structure: {type(msg)}")
            if hasattr(msg, 'keys'):
                print(f"[DEBUG] Message keys: {msg.keys()}")

    def collect_force_data_for_csv(self):
        """Collect current force data for CSV writing"""
        if not self.save_video:
            return
        ...
        try:
            ...
            # Collect uskin force data
            with self.store.lock:
                if (self.store.ros_force_x and self.store.ros_force_y and self.store.ros_force_z):
                    uskin_fx = self.store.ros_force_x[-1] if self.store.ros_force_x else 0.0
                    uskin_fy = self.store.ros_force_y[-1] if self.store.ros_force_y else 0.0  
                    uskin_fz = self.store.ros_force_z[-1] if self.store.ros_force_z else 0.0
                    
                    if len(self.csv_data_queues['uskin_force']) < 10000:
                        self.csv_data_queues['uskin_force'].append([timestamp, uskin_fx, uskin_fy, uskin_fz])
                
                # Collect nano17 force data
                if (self.store.nano17_force_x and self.store.nano17_force_y and self.store.nano17_force_z):
                    nano_fx = self.store.nano17_force_x[-1] if self.store.nano17_force_x else 0.0
                    nano_fy = self.store.nano17_force_y[-1] if self.store.nano17_force_y else 0.0
                    nano_fz = self.store.nano17_force_z[-1] if self.store.nano17_force_z else 0.0
                    
                    if len(self.csv_data_queues['nano17_force']) < 10000:
                        self.csv_data_queues['nano17_force'].append([timestamp, nano_fx, nano_fy, nano_fz])
                        
        except Exception as e:
            print(f"[ERROR] Collecting force data for CSV: {e}")
    ...
    def update_data(self):
        now_time_time = time.time()  # 用于绘图/相对时轴（不影响录制）
        ...
        self.update_charts_from_cache(now_time_time)
        self.capture_frame_for_video()


        # Collect data for CSV if recording
        if self.save_video:
            self.collect_force_data_for_csv()
            
            # Write CSV data periodically to avoid memory buildup
            try:
                for key in self.csv_data_queues:
                    queue_len = len(self.csv_data_queues[key])
                    if queue_len > 100:  # Keep queue manageable
                        rows_written = 0
                        while len(self.csv_data_queues[key]) > 50:  # Keep some buffer
                            row_data = self.csv_data_queues[key].popleft()
                            if key in self.csv_writers:
                                self.csv_writers[key].writerow(row_data)
                                rows_written += 1
                        if rows_written > 0:
                            self.csv_files[key].flush()  # Ensure data is written
                            if key == 'uskin_raw':  # Debug output for raw data
                                print(f"[DEBUG] Wrote {rows_written} rows to {key}, queue was {queue_len}")
    ...
    def update_charts_from_cache(self, current_time):
        ...
        def to_points_ros(ts, xs):
            return [QPointF(t, x*3) for t, x in zip(ts, xs)]

        self.series_nano17_fx.replace(to_points(*c['nx']))
        self.series_ros_fx.replace(to_points_ros(*c['rx']))

        self.series_nano17_fy.replace(to_points(*c['ny']))
        self.series_ros_fy.replace(to_points_ros(*c['ry']))
...
def main():
    ...
    visualizer = ForceVisualizer(ros_sensor_type=args.sensor, video_width=args.width, video_height=args.height,video_name=args.video_name)
    visualizer.show()
    sys.exit(app.exec_())
