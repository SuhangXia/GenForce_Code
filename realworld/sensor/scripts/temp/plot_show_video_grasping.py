...
def plot_window_main(sensor1, sensor2, client, save_video, video_filename, video_fps, video_width, video_height):
    ...
    # sensor1 FZ
    pw1 = win.addPlot(row=0, col=0, title=f'{sensor1} FZ')
    pw1.setLabels(left='N', bottom='t(s)')
    curve1z = pw1.plot(pen=pg.mkPen((0, 0, 180), width=3), name=f'{sensor1} z')

    # sensor2 FZ
    pw2 = win.addPlot(row=0, col=1, title=f'{sensor2} FZ')
    pw2.setLabels(left='N', bottom='t(s)')
    curve2z = pw2.plot(pen=pg.mkPen((0, 0, 180), width=3), name=f'{sensor2} z')

    # sensor1 FX / FY
    pw3 = win.addPlot(row=1, col=0, title=f'{sensor1} FX (Blue) / FY (Green)')
    pw3.setLabels(left='N', bottom='t(s)')
    curve1x = pw3.plot(pen=pg.mkPen((200, 0, 0), width=3), name=f'{sensor1} x')
    curve1y = pw3.plot(pen=pg.mkPen((0, 150, 0), width=3), name=f'{sensor1} y')

    # sensor2 FX / FY
    pw4 = win.addPlot(row=1, col=1, title=f'{sensor2} FX (Blue) / FY (Green)')
    pw4.setLabels(left='N', bottom='t(s)')
    curve2x = pw4.plot(pen=pg.mkPen((200, 0, 0), width=3), name=f'{sensor2} x')
    curve2y = pw4.plot(pen=pg.mkPen((0, 150, 0), width=3), name=f'{sensor2} y')
    ...
    timer.start(max(5, int(1000 / video_fps))) 
    try:
        app.exec_()
    except KeyboardInterrupt:
        pass
...
def main():
    ...
    parser.add_argument('--width', type=int, default=1000, help='Video width (pixels)')
    parser.add_argument('--height', type=int, default=700, help='Video height (pixels)')
