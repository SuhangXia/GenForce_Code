#!/usr/bin/env python3
"""Sequence-oriented multiscale tactile dataset generator.
...
SEQUENCE_VARIANT = "static_multiscale_sequence_v1"
...
def compute_fixed_camera_distance_m(reference_scale_mm: float, base_fov_deg: float, distance_safety: float) -> float:
    ref_scale_m = float(reference_scale_mm) / 1000.0
    fit_scale_m = legacy.compute_full_sensor_fit_scale_m(
        ref_scale_m,
        legacy.DEFAULT_IMAGE_RES[0],
        legacy.DEFAULT_IMAGE_RES[1],
    )
    ref_fov_rad = math.radians(float(base_fov_deg))
    distance = (fit_scale_m / 2.0) / math.tan(ref_fov_rad / 2.0)
...
def compute_camera_params_for_scale(
    *,
    ...
    if camera_mode == "fixed_distance_variable_fov":
        ...
        fov_deg = math.degrees(2.0 * math.atan((fit_scale_m / 2.0) / distance_m))
        return distance_m, float(fov_deg)
    if camera_mode == "fixed_fov_variable_distance":
        fov_deg = float(base_fov_deg)
        distance_m = (fit_scale_m / 2.0) / math.tan(math.radians(fov_deg) / 2.0)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--scales-mm", type=float, nargs="*", default=[float(v) for v in legacy.DEFAULT_SCALES_MM])
    p.add_argument("--scale-min-mm", type=float, default=None)
    p.add_argument("--scale-max-mm", type=float, default=None)
    ...
    p.add_argument("--heldout-scales-mm", type=float, nargs="*", default=None)
    p.add_argument("--heldout-scale-band-mm", type=float, default=0.0)
    p.add_argument("--heldout-val-scales-mm", type=float, nargs="*", default=None)
    p.add_argument("--heldout-test-scales-mm", type=float, nargs="*", default=None)
    p.add_argument("--heldout-val-scale-band-mm", type=float, default=0.0)
    p.add_argument("--heldout-test-scale-band-mm", type=float, default=0.0)
    p.add_argument("--train-scale-intervals-mm", nargs="*", default=None)
    p.add_argument("--val-scale-intervals-mm", nargs="*", default=None)
    p.add_argument("--test-scale-intervals-mm", nargs="*", default=None)
    ...
    p.add_argument("--reference-scale-mm", type=float, default=legacy.DEFAULT_REFERENCE_SCALE_MM)
    ...
    p.add_argument("--uv-mode", choices=["unwrap_genforce", "physical_math"], default="unwrap_genforce")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.scale_mode == "discrete":
        if not args.scales_mm:
            raise ValueError("--scales-mm is required in discrete mode")
    else:
        if args.scale_min_mm is None or args.scale_max_mm is None:
            raise ValueError("--scale-min-mm and --scale-max-mm are required in continuous modes")
        if args.scale_max_mm <= args.scale_min_mm:
            raise ValueError("--scale-max-mm must be > --scale-min-mm")
...
def write_scaled_config_float(base_cfg: Path, out_cfg: Path, scale_mm: float) -> None:
    ...
    try:
        data["elastomer"]["size"]["l"] = float(scale_mm)
        data["elastomer"]["size"]["w"] = float(scale_mm)
    ...
    *,
    ...
    for indenter in indenter_names:
        ...
        for requested_sample_type in requested_sample_types_by_indenter[indenter]:
            ...
            clean_bounds = legacy.compute_margin_constrained_normalized_sampling_bounds(
                bbox_mm=bbox_mm,
                scale_mm=contact_constraint_scale_mm,
                reference_scale_mm=position_reference_scale_mm,
            ...
            inside_bounds_min_scale = legacy.compute_margin_constrained_normalized_sampling_bounds(
                bbox_mm=bbox_mm,
                scale_mm=contact_constraint_scale_mm,
                reference_scale_mm=position_reference_scale_mm,
                requested_x_min_mm=float(args.x_min),
            ...
            inside_bounds_reference_scale = legacy.compute_margin_constrained_normalized_sampling_bounds(
                bbox_mm=bbox_mm,
                scale_mm=position_reference_scale_mm,
                reference_scale_mm=position_reference_scale_mm,
                requested_x_min_mm=float(args.x_min),
                requested_x_max_mm=float(args.x_max),
                requested_y_min_mm=float(args.y_min),
            ...
            sampled_contact = legacy.sample_stratified_contact_position(
                rng=rng,
            ...
            for scale_entry in scale_entries:
                ...
                contact_x_mm, contact_y_mm = legacy.contact_mm_from_normalized(contact_x_norm, contact_y_norm, simulated_mm)
                ...
                episode_state.scales[scale_key] = {
                    ...
                    "adapter_coord_map_shape": [int(v) for v in legacy.make_adapter_coord_map(simulated_mm, patch_h, patch_w).shape],
...
def main() -> None:
    ...
    validate_args(args)
    logging.info(
        "Starting multiscale sequence generation | profile=%s scale_mode=%s dataset_root=%s",
        args.dataset_profile,
        args.scale_mode,
        args.dataset_root,
    )
