...
from sccwm_force_grounded_v21a.losses import compute_force_grounded_v21a_losses, summarize_force_grounded_v21a_batch
...
STAGE_NAME = "stage2_force_grounded_v21a"


def build_force_grounded_v21a_model(cfg: dict[str, Any], device: torch.device) -> SCCWMForceGroundedV21A:
...
def _load_init_weights(model: torch.nn.Module, path: str | Path, device: torch.device) -> dict[str, Any]:
    ...
    print(
        f"[{STAGE_NAME}] init_from={path} loaded {compatible_count} compatible tensors "
        f"(missing={len(missing)} unexpected={len(unexpected)})"
    )
    return payload


def _first_batch(loader: DataLoader[Any]) -> dict[str, Any]:
    for batch in loader:
        return batch
    raise RuntimeError("Expected loader to yield at least one batch for preflight sanity checks")


def _preflight_forward_metrics(
    model: SCCWMForceGroundedV21A,
    batch: dict[str, Any],
    ...
    with torch.no_grad():
        ...
        loss_out = compute_force_grounded_v21a_losses(outputs=outputs, batch=batch, loss_cfg=loss_cfg)
    return {key: float(value.item()) for key, value in loss_out.metrics.items()}


def _print_preflight_summary(label: str, summary: dict[str, Any], forward_metrics: dict[str, float]) -> None:
...
def run_force_grounded_v21a_epoch(
    model: SCCWMForceGroundedV21A,
...
def train_force_grounded_stage2_v21a(cfg: dict[str, Any], *, sanity_only: bool = False) -> None:
    ...
    model = build_force_grounded_v21a_model(cfg, device)
    ...
    train_preflight_summary = summarize_force_grounded_v21a_batch(train_preflight_batch, loss_cfg)
    val_preflight_summary = summarize_force_grounded_v21a_batch(val_preflight_batch, loss_cfg)
    train_preflight_forward = _preflight_forward_metrics(model, train_preflight_batch, loss_cfg, device)
    val_preflight_forward = _preflight_forward_metrics(model, val_preflight_batch, loss_cfg, device)
    preflight_report = {
        "stage": STAGE_NAME,
        "train": {
            "summary": train_preflight_summary,
            "forward_metrics": train_preflight_forward,
        },
        "val": {
            "summary": val_preflight_summary,
            "forward_metrics": val_preflight_forward,
        },
    }
    dump_json(preflight_report, output_dir / "preflight_sanity.json")
    _print_preflight_summary("train", train_preflight_summary, train_preflight_forward)
    _print_preflight_summary("val", val_preflight_summary, val_preflight_forward)
    if sanity_only:
        dump_json(
            {
                "stage": STAGE_NAME,
                "status": "sanity_only_completed",
                "output_dir": str(output_dir),
            },
            output_dir / "sanity_only_summary.json",
        )
    ...
    wandb.init(
        ...
        name=str(cfg.get("system", {}).get("wandb_name", cfg.get("experiment", {}).get("name", "sccwm_force_grounded_v21a"))),
    ...
    try:
        for epoch in range(start_epoch, total_epochs + 1):
            ...
            train_metrics = run_force_grounded_v21a_epoch(
                model,
                train_loader,
                optimizer,
                device,
            ...
            with torch.no_grad():
                val_metrics = run_force_grounded_v21a_epoch(
                    model,
                    val_loader,
                    None,
                    device,
...
def main() -> None:
    parser = build_stage_argparser(
        "Train the force-grounded SCCWM v2.1a stage-2 model.",
        "sccwm_force_grounded_v21a/configs/sccwm_stage2_force_grounded_v21a.yaml",
    )
    parser.add_argument(
        "--sanity-only",
        action="store_true",
        help="Run preflight sanity checks on the first train/val batch and exit without training.",
    )
    args = parser.parse_args()
    cfg = load_config_with_overrides(args.config, args.override)
    train_force_grounded_stage2_v21a(cfg, sanity_only=bool(args.sanity_only))
