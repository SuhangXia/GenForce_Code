...
REGRESSOR_CKPT="${REGRESSOR_CKPT:-/datasets/checkpoints/standalone_stea_experiments/no_adapter_baseline/best.pt}"
if [ ! -f "$REGRESSOR_CKPT" ] && [ -f "/home/suhang/datasets/checkpoints/standalone_stea_experiments/no_adapter_baseline/best.pt" ]; then
  REGRESSOR_CKPT="/home/suhang/datasets/checkpoints/standalone_stea_experiments/no_adapter_baseline/best.pt"
fi
...
mkdir -p "$RESULTS_DIR"
LOG_PATH="$RESULTS_DIR/no_adapter_baseline_matrix.txt"
: > "$LOG_PATH"
