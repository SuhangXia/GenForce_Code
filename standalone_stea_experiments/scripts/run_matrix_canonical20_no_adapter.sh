...
REGRESSOR_CKPT="${REGRESSOR_CKPT:-/datasets/checkpoints/standalone_stea_experiments/canonical20_no_adapter/best.pt}"
if [ ! -f "$REGRESSOR_CKPT" ] && [ -f "/home/suhang/datasets/checkpoints/standalone_stea_experiments/canonical20_no_adapter/best.pt" ]; then
  REGRESSOR_CKPT="/home/suhang/datasets/checkpoints/standalone_stea_experiments/canonical20_no_adapter/best.pt"
fi
...
mkdir -p "$RESULTS_DIR"
LOG_PATH="$RESULTS_DIR/canonical20_no_adapter_matrix.txt"
: > "$LOG_PATH"
