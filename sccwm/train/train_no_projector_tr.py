...
from sccwm.models import NoProjectorTR
...
def build_no_projector_tr_model(cfg: dict[str, Any], device: torch.device) -> NoProjectorTR:
    model_cfg = cfg.get("model", {})
    model = NoProjectorTR(
        input_channels=int(model_cfg.get("input_channels", 3)),
        feature_dim=int(model_cfg.get("feature_dim", 128)),
        hidden_dim=int(model_cfg.get("world_hidden_dim", 128)),
    )
...
def _compute_no_projector_tr_losses(
...
def run_no_projector_tr_epoch(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(iterator):
        ...
        total_loss, metrics_t = _compute_no_projector_tr_losses(outputs, batch, loss_cfg)
...
def train_no_projector_tr(cfg: dict[str, Any]) -> None:
    ...
    model = build_no_projector_tr_model(cfg, device)
    ...
    wandb.init(
        ...
        name=str(cfg.get("system", {}).get("wandb_name", cfg.get("experiment", {}).get("name", "no_projector_tr"))),
        config=cfg,
    )
    ...
    try:
        for epoch in range(start_epoch, total_epochs + 1):
            ...
            if epoch_episode_subset is not None:
                print(
                    f"[no_projector_tr] epoch={epoch} sampled {len(epoch_episode_subset)} train episodes for this epoch "
            ...
            train_metrics = run_no_projector_tr_epoch(
                model,
                train_loader,
                optimizer,
                device,
                loss_cfg=loss_cfg,
                desc=f"Train no_projector_tr {epoch}",
                progress=not bool(train_cfg.get("no_progress", False)),
                max_batches=max_train_batches,
            )
            with torch.no_grad():
                val_metrics = run_no_projector_tr_epoch(
                    model,
                    val_loader,
                    None,
                    device,
                    loss_cfg=loss_cfg,
                    desc=f"Val no_projector_tr {epoch}",
                    progress=not bool(train_cfg.get("no_progress", False)),
            ...
            print(f"[no_projector_tr] saved last checkpoint -> {output_dir / 'last.pt'}")
            if improved:
                best_value = current
                payload["best_value"] = best_value
                save_checkpoint(output_dir / "best.pt", payload)
                print(
                    f"[no_projector_tr] updated best checkpoint -> {output_dir / 'best.pt'} "
            ...
            if int(train_cfg.get("save_every", 0)) > 0 and epoch % int(train_cfg.get("save_every", 1)) == 0:
                ...
                print(f"[no_projector_tr] saved periodic checkpoint -> {output_dir / f'epoch_{epoch:04d}.pt'}")
            ...
            print(
                f"[no_projector_tr] epoch={epoch} train_loss={train_metrics.get('loss', 0.0):.4f} "
...
def main() -> None:
    parser = build_stage_argparser("Train the NoProjectorTR image-space temporal baseline.", "sccwm/configs/no_projector_tr.yaml")
    args = parser.parse_args()
    cfg = load_config_with_overrides(args.config, args.override)
    train_no_projector_tr(cfg)
