...
def load_no_projector_tr_for_eval(cfg: dict[str, Any], checkpoint_path: str | Path, device: torch.device) -> NoProjectorTR:
...
def run_no_projector_tr_eval(
    ...
    *,
    ...
    model = load_no_projector_tr_for_eval(cfg, checkpoint_path, device)
    loader = build_eval_loader(cfg, split=split, sequence_length=sequence_length)
    records: list[dict[str, Any]] = []
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"NoProjectorTR eval {protocol} {split}", total=len(loader), leave=False)
...
def main() -> None:
    parser = build_eval_argparser("Evaluate the NoProjectorTR baseline.", "sccwm/configs/no_projector_tr.yaml")
    parser.add_argument(
        "--protocol",
        type=str,
    ...
    result = run_no_projector_tr_eval(
        cfg,
    ...
    output = args.output or f"sccwm/eval_outputs/no_projector_tr_{args.protocol}_{args.split}.json"
