...
from sccwm_force_grounded_v25.models import EMBEDDING_VIEW_TO_KEY, build_force_grounded_v25_model, select_embedding_view_v25
...
def load_force_grounded_v25_for_eval(cfg: dict[str, Any], checkpoint_path: str | Path, device: torch.device) -> torch.nn.Module:
    model = build_force_grounded_v25_model(cfg.get("model", {})).to(device)
...
def _run_force_grounded_eval_loop_v25(
    *,
    cfg: dict[str, Any],
    ...
    with torch.no_grad():
        iterator = tqdm(loader, desc=f"ForceGroundedV25 direct eval {protocol} {split_label}", total=len(loader), leave=False)
        prefix = "force_grounded_v25_cross_band_eval" if strict_cross_band else "force_grounded_v25_direct_eval"
        _progress_write(
            iterator,
...
def run_force_grounded_direct_eval_v25(
    cfg: dict[str, Any],
    *,
    checkpoint_path: str | Path,
    split: str,
    ...
    if protocol in STRICT_CROSS_BAND_PROTOCOLS:
        ...
        result = _run_force_grounded_eval_loop_v25(
            cfg=cfg,
            model=model,
            loader=loader,
            protocol=protocol,
...
def main() -> None:
    parser = build_eval_argparser(
        "Evaluate split-canonical force-guided SCCWM v25 direct prediction protocols.",
    ...
    result = run_force_grounded_direct_eval_v25(
        cfg,
    ...
    output = args.output or f"sccwm/eval_outputs/sccwm_force_grounded_v25_{args.protocol}_{args.split}_{args.embedding_view}.json"
