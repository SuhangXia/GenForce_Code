...
from sccwm_force_grounded_v25.losses import compute_force_grounded_v25_losses, summarize_force_grounded_v25_batch
from sccwm_force_grounded_v25.models import build_force_grounded_v25_model
...
def _stage_name(cfg: dict[str, Any]) -> str:
    return str(cfg.get("experiment", {}).get("name", "stage2_force_grounded_v25scfgof"))


def build_force_grounded_v25_model_for_train(cfg: dict[str, Any], device: torch.device) -> torch.nn.Module:
    return build_force_grounded_v25_model(cfg.get("model", {})).to(device)


def _preflight_forward_metrics_v25(
    ...
    with torch.no_grad():
        ...
        loss_out = compute_force_grounded_v25_losses(outputs=outputs, batch=batch, loss_cfg=loss_cfg)
    return {key: float(value.item()) for key, value in loss_out.metrics.items()}


def _print_preflight_summary_v25(stage_name: str, label: str, summary: dict[str, Any], forward_metrics: dict[str, float]) -> None:
    ...
    print(
        f"[{stage_name}] preflight {label}: force_guidance_weight={summary.get('force_guidance_weight', float('nan')):.4f} "
        f"enable_split_canonical={bool(round(forward_metrics.get('split_canonical_enabled', 1.0)))}"
    )
    ...
    print(
        ...
        f"load_scale_adv_acc={forward_metrics.get('load_scale_adv_acc', float('nan')):.4f} "
        f"load_branch_adv_acc={forward_metrics.get('load_branch_adv_acc', float('nan')):.4f} "
        f"pose_scale_adv_acc={forward_metrics.get('pose_scale_adv_acc', float('nan')):.4f} "
        f"pose_branch_adv_acc={forward_metrics.get('pose_branch_adv_acc', float('nan')):.4f} "
        ...
        f"canonical_force_alignment={forward_metrics.get('canonical_force_alignment', float('nan')):.4f} "
        f"canonical_force_similarity_distill={forward_metrics.get('canonical_force_similarity_distill', float('nan')):.4f} "
        f"force_weighted_load_pair_consistency={forward_metrics.get('force_weighted_load_pair_consistency', float('nan')):.4f}"
    )


def run_force_grounded_v25_epoch(
    stage_name: str,
    model: torch.nn.Module,
...
def train_force_grounded_stage2_v25(cfg: dict[str, Any], *, sanity_only: bool = False) -> None:
    ...
    model = build_force_grounded_v25_model_for_train(cfg, device)
    ...
    train_preflight_summary = summarize_force_grounded_v25_batch(train_preflight_batch, loss_cfg)
    val_preflight_summary = summarize_force_grounded_v25_batch(val_preflight_batch, loss_cfg)
    train_preflight_forward = _preflight_forward_metrics_v25(model, train_preflight_batch, loss_cfg, device)
    val_preflight_forward = _preflight_forward_metrics_v25(model, val_preflight_batch, loss_cfg, device)
    preflight_report = {
        "stage": stage_name,
        ...
        "model": {
            "enable_swap_decode": bool(cfg.get("model", {}).get("enable_swap_decode", True)),
            "enable_split_canonical": bool(cfg.get("model", {}).get("enable_split_canonical", True)),
        },
    }
    dump_json(preflight_report, output_dir / "preflight_sanity.json")
    _print_preflight_summary_v25(stage_name, "train", train_preflight_summary, train_preflight_forward)
    _print_preflight_summary_v25(stage_name, "val", val_preflight_summary, val_preflight_forward)
    if sanity_only:
    ...
    try:
        for epoch in range(start_epoch, total_epochs + 1):
            ...
            train_metrics = run_force_grounded_v25_epoch(
                stage_name,
                model,
                train_loader,
                optimizer,
            ...
            with torch.no_grad():
                val_metrics = run_force_grounded_v25_epoch(
                    stage_name,
                    model,
                    val_loader,
                    None,
...
def main() -> None:
    parser = build_stage_argparser(
        "Train split-canonical force-guided geometry/operator factorized SCCWM v25.",
    ...
    train_force_grounded_stage2_v25(cfg, sanity_only=bool(args.sanity_only))
